...
async function fetchOneEndpoint(id: string) {
  const response = await fetch(`${config.webAPI.hostURL}${config.webAPI.basePathLLMEndpoints}/${id}`,
  { cache: 'no-store' })
  const result = await processResponse<LLMEndpoint>(response);
  return result;
}

export default async function CreateNewEndpointPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="flex flex-col pt-4 w-full h-full">
      <NewEndpointForm disablePopupLayout endpointToEdit={result.data} />
    </div>
  );
}
